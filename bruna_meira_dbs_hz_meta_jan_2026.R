...
# LOW|VERY LOW vs HIGH Data pre-processing  -------------------------
...
# LOW|VERY LOW vs HIGH Overall and Main Domains  ------------
...
nrow(low_high_df) # 89 contrasts

length(unique(low_high_df$study_name)) # 20 studies
...
results %>%
  ...
  labs(
    x = "\n Assumed correlation r",
    y = "Effect size (Hedges' g) \n",
    title = "Low|Very Low Hz vs High Hz \nSensitivity of effect size to assumed correlation r"
  ) +
  theme(axis.text.y = element_blank(),
        axis.ticks.y = element_blank(),
        legend.position = "none") +
...
# LOW|VERY LOW vs HIGH Main Verbal Fluency Sub Domains  ------------
...
nrow(low_high_df) # 33 contrast
length(unique(low_high_df$study_name)) # 9 studies
...
# -----------------------------
# LOW|VERY LOW vs HIGH Executive Function  ------------
...
nrow(low_high_df) #61
length(unique(low_high_df$study_name)) #16
...
# -----------------------------
# LOW|VERY LOW vs HIGH Secondary Domains  ------------
...
nrow(low_high_df) # 74 contrast
length(unique(low_high_df$study_name)) # 16 studies
...
for(d in valid_domains){
  ...
  forest(res,
         slab = paste(df$study_name, "-", df$`Cog Task`),
         xlab = "Hedges' g (positive = better cognitive performance for [Low|Very LowHz])",
         main = paste("\n Forest plot:", d),
         annotate=TRUE, addfit=TRUE, 
         showweights=TRUE, header=TRUE,
         cex = 0.9,
...
# LOW|VERY LOW VS HIGH Radial plots summary horizontal -------
...
plt <- plt +
  ...
  scale_fill_manual(
    values = c(
      "Positive [Better Low|Very Low Hz]" = "#0c3647",
      "Negative [Better High Hz]" = "#a82258"
    )
  ) +
  scale_colour_manual(
...
ggsave(file="plt_lowvlow_prim_naive.svg", plot=plt, width=8, height=5)
...
ggsave(file="plt_lowvlow_prim_mlma.svg", plot=plt, width=8, height=5)
...
ggsave(file="plt_lowvlow_sec_naive.svg", plot=plt, width=8, height=5)
...
ggsave(file="plt_lowvlow_sec_mlma.svg", plot=plt, width=8, height=5)
...
# LOW|VERY LOW VS HIGH Radial plots summary circular -------
...
# OFF vs HIGH Data pre-processing  -------------------------
...
# OFF vs HIGH Overall and Main Domains  ------------


unique(final_df$condition_A)
unique(final_df$condition_B)
...
nrow(off_high_df) # 63 contrasts

length(unique(off_high_df$study_name)) # 1 studies
...
tau2 <- res_lh$sigma2
I2 <- 100 * tau2 / (tau2 + mean(off_high_df$vi))
...
meta::forest(
  ...
  annotate=TRUE, addfit=TRUE, 
  showweights=TRUE, header=TRUE,
  slab = paste(off_high_df$study_name, "-", off_high_df$`Cog Task`),
  xlab = "Hedges' g (positive = better cognitive performance for [OFF Hz])",
  cex = 0.9,
  col="white", border="firebrick",
  colout="firebrick"
...
table(off_high_df$Main_cog_domain)


# Identify domains with at least 2 effect sizes
domain_counts <- table(off_high_df$Main_cog_domain)
valid_domains <- names(domain_counts[domain_counts >= 2])
...
for(d in valid_domains){
  ...
  df <- off_high_df %>% filter(Main_cog_domain == d)
...
studies <- unique(off_high_df$study_name)
...
loso_results <- lapply(studies, function(s) {
  ...
  res <- rma.mv(
    ...
    random = ~ 1 |study_name /  effect_id,
    method = "REML",
    data = subset(off_high_df, study_name != s)
  )
...
# -----------------------------
# OFF vs HIGH Main Verbal Fluency Sub Domains  ------------


off_high_df <- final_df %>%
  filter( (condition_A == "Off" & condition_B == "HighHz")  ) %>% filter(!is.na(Main_Subdomain))

table(off_high_df$condition_A, off_high_df$condition_B)

nrow(off_high_df) # 35 contrast
length(unique(off_high_df$study_name)) # 6 studies

table(off_high_df$Main_Subdomain)


# Identify domains with at least 2 effect sizes
domain_counts <- table(off_high_df$Main_Subdomain)
valid_domains <- names(domain_counts[domain_counts >= 2])
...
for(d in valid_domains){
  ...
  df <- off_high_df %>% filter(Main_Subdomain == d)
...
300
# -----------------------------
# OFF vs HIGH Executive Function  ------------
...
nrow(off_high_df) #61
length(unique(off_high_df$study_name)) #16
...
# -----------------------------
# OFF vs HIGH Secondary Domains  ------------


off_high_df <- final_df %>%
     filter( (condition_A == "Off" & condition_B == "HighHz")) %>% filter(!is.na(Secondary_Domain))


table(off_high_df$condition_A, off_high_df$condition_B)

nrow(off_high_df) # 107 contrast
length(unique(off_high_df$study_name)) # 11 studies

table(off_high_df$Secondary_Domain)


# Identify domains with at least 2 effect sizes
domain_counts <- table(off_high_df$Secondary_Domain)
valid_domains <- names(domain_counts[domain_counts >= 2])
...
for(d in valid_domains){
  ...
  df <- off_high_df %>% filter(Secondary_Domain == d)
...
400
# -----------------------------
# OFF vs HIGH - Any Domain Across Main+Secondary+Terteary ----------

final_df_new_flags_group_paulo_jan_17 <- fread("final_df_new_flags_group_paulo_jan_17.csv")

off_high_df <- final_df_new_flags_group_paulo_jan_17 %>%
  filter(  (condition_A == "Off" & condition_B == "HighHz") ) %>%
  select(study_name, study_id, effect_id, yi, vi, `Cog Task`, `Verbal Fluency`:`Attention`)

off_high_df <- off_high_df %>% gather(key="Domain", value="ON", `Verbal Fluency`:`Attention`) %>%
  drop_na() %>%
  arrange(study_name, study_id, effect_id, Domain) %>% select(-ON)


length(unique(off_high_df$study_id)) 
length(unique(off_high_df$study_name)) 


table(off_high_df$Domain)



# Identify domains with at least 2 effect sizes
domain_counts <- table(off_high_df$Domain)
valid_domains <- names(domain_counts[domain_counts >= 2])
...
for(d in valid_domains){
  ...
  df <- off_high_df %>% filter(Domain == d)
...
# OFF VS HIGH Radial plots summary horizontal -------
...
# OFF VS HIGH Radial plots summary circular -------
...
# OFF vs LOW|VERY LOW Data pre-processing  -------------------------
...
# OFF vs LOW|VERY LOW Overall and Main Domains  ------------
...
nrow(off_low_df) # 63 contrasts

length(unique(off_low_df$study_name)) # 1 studies
...
tau2 <- res_lh$sigma2
I2 <- 100 * tau2 / (tau2 + mean(off_low_df$vi))
...
meta::forest(
  ...
  annotate=TRUE, addfit=TRUE, 
  showweights=TRUE, header=TRUE,
  slab = paste(off_low_df$study_name, "-", off_low_df$`Cog Task`),
  xlab = "Hedges' g (positive = better cognitive performance for [OFF])",
  cex = 0.9,
  col="white", border="firebrick",
  colout="firebrick"
...
table(off_low_df$Main_cog_domain)
...
studies <- unique(off_low_df$study_name)
...
loso_results <- lapply(studies, function(s) {
  ...
  res <- rma.mv(
    ...
    random = ~ 1 | study_name / effect_id,
    method = "REML",
    data = subset(off_low_df, study_name != s)
  )
...
# -----------------------------
# OFF vs LOW|VERY LOW Main Verbal Fluency Sub Domains  ------------
...
300
# -----------------------------
# OFF vs LOW|VERY LOW Executive Function  ------------
...
# -----------------------------
# OFF vs LOW|VERY LOW Secondary Domains  ------------
...
400
# -----------------------------
# OFF vs LOW|VERY LOW - Any Domain Across Main+Secondary+Terteary ----------
...
# OFF VS LOW|VERY LOW Radial plots summary horizontal -------
...
plot_df_offlow_primary_naive <- tibble(
  domain = c(
    "Verbal Fluency (naïve)",
    "Time Processing (naïve)",
    "Processing Speed (naïve)",
  ...
  g = c(
    -0.166,  # Verbal Fluency
    -0.289,  # Time Processing
     0.161,  # Processing Speed
    -0.378,  # Executive Control
    -0.144,  # Cognitive Flexibility
    -0.121,  # Working Memory
    -0.190   # Attention
  ),
  ci_low = c(
    -0.311,  # Verbal Fluency
    -0.774,  # Time Processing
    -0.370,  # Processing Speed
    -0.638,  # Executive Control
    -0.344,  # Cognitive Flexibility
    -0.354,  # Working Memory
    -0.456   # Attention
  ),
  ci_high = c(
    -0.0213, # Verbal Fluency
     0.196,  # Time Processing
     0.691,  # Processing Speed
    -0.119,  # Executive Control
     0.0552, # Cognitive Flexibility
     0.113,  # Working Memory
     0.0767  # Attention
  )
)


plot_df2 <- plot_df_offlow_primary_naive %>%
 # arrange(g) %>%
  mutate(
    domain_wrapped = str_wrap(domain, 20),
    Direction = ifelse(g >= 0, "Positive [Better OFF]", "Negative [Better ON]")  )
...
plt <- ggplot(plot_df2) +
  geom_hline( aes(yintercept = y),
    data = data.frame(y = seq(-1.0, 1.0, 0.1)[seq(-1.0, 1.0, 0.1) != 0]),
    color = "lightgrey", linewidth = 0.4) +
  geom_hline( yintercept = 0, color = "gray12", linewidth = 1.2) +
  geom_col(aes(x = domain_wrapped,
      y = g,
...
ggsave(file="plt_offlow_prim_naive.svg", plot=plt, width=8, height=5)
...
plot_df_offlow_primary_mlma <- tibble(
  domain = c(
    "Verbal Fluency (MLMA)",
    "Time Processing (MLMA)",
    "Processing Speed (MLMA)",
  ...
  g = c(
    -0.202,  # Verbal Fluency
    -0.289,  # Time Processing
     0.161,  # Processing Speed
    -0.373,  # Executive Control
    -0.144,  # Cognitive Flexibility
    -0.121,  # Working Memory
    -0.0921  # Attention
  ),
  ci_low = c(
    -0.399,  # Verbal Fluency
    -0.774,  # Time Processing
    -0.370,  # Processing Speed
    -0.682,  # Executive Control
    -0.344,  # Cognitive Flexibility
    -0.354,  # Working Memory
    -0.429   # Attention
  ),
  ci_high = c(
    -0.00456, # Verbal Fluency
     0.196,   # Time Processing
     0.691,   # Processing Speed
    -0.0636,  # Executive Control
     0.0552,  # Cognitive Flexibility
     0.113,   # Working Memory
     0.245    # Attention
  )
...
plot_df2 <- plot_df_offlow_primary_mlma %>%
 # arrange(g) %>%
  mutate(
    domain_wrapped = str_wrap(domain, 20),
    Direction = ifelse(g >= 0, "Positive [Better OFF]", "Negative [Better ON]")  )
...
ggsave(file="plt_offlow_prim_mlma.svg", plot=plt, width=8, height=5)
...
plot_df_offlow_sec_naive <- tibble(
  domain = c(
    "Verbal Fluency (naïve)",
    "Processing Speed (naïve)",
    "Executive Control (naïve)",
  ...
  g = c(
    -0.188,  # Verbal Fluency
    -0.0684, # Processing Speed
     0.0768, # Executive Control
    -0.166,  # Cognitive Flexibility
    -0.323,  # Top-down Attention
    -0.120   # Attention
  ),
  ci_low = c(
    -0.434,  # Verbal Fluency
    -0.265,  # Processing Speed
    -2.10,   # Executive Control
    -0.311,  # Cognitive Flexibility
    -0.570,  # Top-down Attention
    -0.317   # Attention
  ),
  ci_high = c(
     0.0589, # Verbal Fluency
     0.128,  # Processing Speed
     2.25,   # Executive Control
    -0.0213, # Cognitive Flexibility
    -0.0768, # Top-down Attention
     0.0770  # Attention
  )
)
...
plot_df2 <- plot_df_offlow_sec_naive %>%
 # arrange(g) %>%
  mutate(
    domain_wrapped = str_wrap(domain, 20),
    Direction = ifelse(g >= 0, "Positive [Better OFF]", "Negative [Better ON]")  )
...
ggsave(file="plt_offlow_sec_naive.svg", plot=plt, width=8, height=5)
...
plot_df_offlow_sec_mlma <- tibble(
  domain = c(
    "Verbal Fluency (MLMA)",
    "Processing Speed (MLMA)",
    "Executive Control (MLMA)",
  ...
  g = c(
    -0.188,  # Verbal Fluency
    -0.0415, # Processing Speed
     0.0768, # Executive Control
    -0.202,  # Cognitive Flexibility
    -0.301,  # Top-down Attention
    -0.113   # Attention
  ),
  ci_low = c(
    -0.434,  # Verbal Fluency
    -0.259,  # Processing Speed
    -2.10,   # Executive Control
    -0.399,  # Cognitive Flexibility
    -0.594,  # Top-down Attention
    -0.322   # Attention
  ),
  ci_high = c(
     0.0589, # Verbal Fluency
     0.176,  # Processing Speed
     2.25,   # Executive Control
    -0.00456,# Cognitive Flexibility
    -0.00765,# Top-down Attention
     0.0952  # Attention
  )
)
...
plot_df2 <- plot_df_offlow_sec_mlma %>%
 # arrange(g) %>%
  mutate(
    domain_wrapped = str_wrap(domain, 20),
    Direction = ifelse(g >= 0, "Positive [Better OFF]", "Negative [Better ON]")  )
...
ggsave(file="plt_offlow_sec_mlma.svg", plot=plt, width=8, height=5)
...
# OFF VS LOW|VERY LOW Radial plots summary circular -------
...
plt <- ggplot(plot_df2) + 
  geom_hline( aes(yintercept = y), data = data.frame(y = seq(-1.0, 1.0, 0.1)[seq(-1.0, 1.0, 0.1) != 0]), color = "lightgrey", linewidth = 0.4 ) + 
  geom_hline( yintercept = 0, color = "gray12", linewidth = 1.2 ) + 
  geom_col( aes( x = domain_wrapped, y = g, fill = Direction, alpha=0.7 ), width = 0.9, color = "gray20", show.legend = TRUE ) + 
  geom_segment(data = plot_df2,
    aes(x = domain_wrapped, xend = domain_wrapped,y = ci_low,yend = ci_high),
  ...
  coord_polar(clip = "off") 
plt 
plt <- plt + scale_y_continuous( limits = c(-1.0, 1.0), breaks = seq(-1.0, 1.0, 0.1), expand = c(0, 0) ) + 
  scale_fill_manual( values = c( "Positive [Better OFF]" = "#0c3647", "Negative [Better ON]" = "#a82258" ) ) + 
  scale_alpha_identity() + 
  theme_minimal(base_size = 12) + 
  theme( axis.title = element_blank(), 
...
ggsave(file="plt_offlow_prim_naive_circ.svg", plot=plt, width=5, height=5)
...
ggsave(file="plt_offlow_prim_mlma_circ.svg", plot=plt, width=5, height=5)
...
plot_df2 <- plot_df2 %>% 
  mutate(ci_high=ifelse(ci_high>=1.0,1.0,ci_high)) %>%
  mutate(ci_low=ifelse(ci_low<=-1.0,-1.0,ci_low))

plt <- ggplot(plot_df2) + 
  geom_hline( aes(yintercept = y), data = data.frame(y = seq(-1.1, 1.0, 0.1)[seq(-1.0, 1.0, 0.1) != 0]), color = "lightgrey", linewidth = 0.4 ) + 
  geom_hline( yintercept = 0, color = "gray12", linewidth = 1.2 ) + 
  geom_col( aes( x = domain_wrapped, y = g, fill = Direction, alpha=0.7 ), width = 0.9, color = "gray20", show.legend = TRUE ) + 
  geom_segment(data = plot_df2,
    aes(x = domain_wrapped, xend = domain_wrapped,y = ci_low,yend = ci_high),
...
plt 
plt <- plt + 
  scale_y_continuous( limits = c(-1.0, 1.0), breaks = seq(-1.0, 1.0, 0.1), expand = c(0, 0) ) + 
  scale_fill_manual( values = c( "Positive [Better OFF]" = "#0c3647", "Negative [Better ON]" = "#a82258" ) ) + 
  scale_alpha_identity() + 
  theme_minimal(base_size = 12) + 
  theme( axis.title = element_blank(), 
...
ggsave(file="plt_offlow_sec_naive_naive_circ.svg", plot=plt, width=5, height=5)
...
ggsave(file="plt_offlow_sec_mlma_circ.svg", plot=plt, width=5, height=5)
...
# LOW  VERY LOW BOTH vs HIGH summary horizontal Main+Sec+tert ------- 
# LOW VS HIGH - PRIMARY+SECONDARY+TERTIARY DOMAINS - MLMA

plot_df_low_primary_secondary_tertiary_mlma <- tibble(
  domain = c(
    "Executive Control (MLMA)","Verbal Fluency (MLMA)","Working Memory (MLMA)", "Attention (MLMA)", "Cognitive Flexibility (MLMA)"
  ),
  g = c(-0.01, 0.24, -0.22, 0, 0.24),
  ci_low=c(-0.28,0.02,-0.61 , -0.23, 0.02),
  ci_high=c(0.26,0.45,0.18, 0.23, 0.45)
)




plot_df2 <- plot_df_low_primary_secondary_tertiary_mlma %>%
 # arrange(g) %>%
  mutate(
    domain_wrapped = str_wrap(domain, 20),
    Direction = ifelse(g >= 0, "Positive [Better Low Hz]", "Negative [Better High Hz]")  )
...
ggsave(file="plt_low_prim_sec_tert_mlma.svg", plot=plt, width=5, height=5)
...
# VERYLOW VS HIGH - PRIMARY+SECONDARY+TERTIARY DOMAINS - MLMA


plot_df_lowvlow_primary_secondary_tertiary_mlma <- tibble(
  domain = c("Verbal Fluency (MLMA)",
    "Time Processing (MLMA)",
    "Processing Speed (MLMA)",
    "Executive Control (MLMA)",
    ...
    "Working Memory (MLMA)",
    "Attention (MLMA)"),
  g = c(0.30, -0.12, -0.26, 0.16, 0.30, 0.03, 0.10),
  ci_low=c(0.17, -1.69,-0.70, -0.27, 0.18, -0.79, -0.15),
  ci_high=c(0.43, 1.46, 0.19 , 0.60, 0.43, 0.85, 0.35)
)



plot_df2 <- plot_df_lowvlow_primary_secondary_tertiary_mlma %>%
 # arrange(g) %>%
  mutate(
    domain_wrapped = str_wrap(domain, 20),
    Direction = ifelse(g >= 0, "Positive [Better Very Low Hz]", "Negative [Better High Hz]")  )
...
ggsave(file="plt_vlow_prim_sec_tert_mlma.svg", plot=plt, width=8, height=5)
...
plot_df_lowvlow_primary_sec_ter_mlma <- tibble(
  domain = c("Verbal Fluency (MLMA)",
    "Time Processing (MLMA)",
    "Processing Speed (MLMA)",
    "Executive Control (MLMA)",
    ...
    "Working Memory (MLMA)",
    "Attention (MLMA)"),
  g = c(0.28, -0.12, -0.23, 0.12, 0.28, 0.01, 0.08),
  ci_low=c(0.18, -1.69,-0.66, -0.19, 0.19, -0.63, -0.12),
  ci_high=c(0.37, 1.46, 0.19 , 0.44, 0.37, 0.64, 0.28)
)



plot_df2 <- plot_df_lowvlow_primary_sec_ter_mlma %>%
 # arrange(g) %>%
  mutate(
    domain_wrapped = str_wrap(domain, 20),
    Direction = ifelse(g >= 0, "Positive [Better Low|Very Low Hz]", "Negative [Better High Hz]")  )
...
ggsave(file="plt_lowvlow_prim_sec_tert_mlma.svg", plot=plt, width=8, height=5)
